...
<xsl:stylesheet exclude-result-prefixes="#all" version="2.0" xmlns="http://www.w3.org/1999/xhtml" xmlns:util="enonic:utilities" xmlns:portal="http://www.enonic.com/cms/xslt/portal" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema">
    ...
    <xsl:template match="/">
        <script type="text/javascript">
            $(document).ready(function() {
              ...
              $('#new-customer').hide();
                ...
                var validator = $("#create-new-user").validate({ 
                    rules: { 
                        first_name: "required", 
                        last_name: "required", 
        ...
        <xsl:if test="/result/context/querystring/parameter[@name = 'error_user_login']">
            ...
            <p class="error">An error occured: 
        ...
        <xsl:if test="/result/context/querystring/parameter[@name = 'error_user_create']">
            ...
            <p class="error"> An error occured: <xsl:value-of select="$errorCode"/>
                <xsl:text> - </xsl:text>
                <xsl:choose>
        ...
        <p class="error" id="show-validation-error" style="display:none;"></p>
              ...
              <script type="text/javascript">
                 <xsl:comment>
                    function checkPasswords() {
                       if($('#repeat-new-password').val() == $('#new-password').val()) {
                            $('#repeat-new-password').attr('disabled','disabled');
                       } else {
                            $('#show-validation-error').html('The passwords do not match');
                       }
                    };
                 //</xsl:comment>
              </script>
                
                <form action="{portal:createServicesUrl('user','create', $redirect-create, ())}" method="post" enctype="multipart/form-data" id="create-new-user" onsubmit="checkPasswords()">
                    <input type="hidden" name="userstore" value="{$shopper-userstore}"/>
                    ...
                    <label for="new-password"><xsl:value-of select="portal:localize('Password')"/></label>
                        <input type="password" name="password" id="new-password" class="required" minlength="5"/>
                    <label for="repeat-new-password">Repeat password</label>
                        <input type="password" name="repeat_password" id="repeat-new-password" class="required" minlength="5"/>
