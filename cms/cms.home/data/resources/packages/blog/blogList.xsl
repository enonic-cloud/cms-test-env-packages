...
<xsl:stylesheet exclude-result-prefixes="xs portal" version="2.0" xmlns="http://www.w3.org/1999/xhtml" xmlns:portal="http://www.enonic.com/cms/xslt/portal" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template name="comments">
    <xsl:variable name="comments" select="/verticaldata/contents/relatedcontents/content[categoryname/@key = $categorykey]/contentdata/related/content[@key = current()/@key]"/>
    <xsl:variable name="noOfComments" select="count($comments)"/>
    <xsl:variable name="latestCommentName" select="$comments[position() = last()]/../../../owner/@uid"/>
