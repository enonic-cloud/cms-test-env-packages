...
<xsl:stylesheet exclude-result-prefixes="#all" version="2.0" xmlns="http://www.w3.org/1999/xhtml"
    ...
    <xsl:variable name="filter">
        <xsl:choose>
            <xsl:when test="exists(/result/gsa-search/GSP/RES/FI)">
    ...
    <xsl:variable name="total-count" as="xs:integer">
        <xsl:choose>
            <xsl:when test="/result/gsa-search/GSP/gsa-search/RES/M">
                <xsl:value-of select="/result/gsa-search/GSP/gsa-search/RES/M"/>
            </xsl:when>
            <xsl:otherwise>0</xsl:otherwise>
        </xsl:choose>
    </xsl:variable>
