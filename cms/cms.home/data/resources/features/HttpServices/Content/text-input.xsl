...
<xsl:stylesheet exclude-result-prefixes="#all" version="2.0"
  ...
  <xsl:param name="category-key" select="85"/>
  ...
  <xsl:template name="group-2-template">
    ...
    <div class="block-group">
      <xsl:call-template name="text-input">
        <xsl:with-param name="name" select="concat('Entries-1[', $position ,'].entries1_required')"/>
        <xsl:with-param name="value" select="$select-node/required"/>
        <xsl:with-param name="label" select="'Required'"/>
        <xsl:with-param name="required" select="true()"/>
      </xsl:call-template>
      <xsl:call-template name="text-input">
        <xsl:with-param name="name" select="concat('Entries-1[', $position ,'].entries1_unrequired')"/>
  ...
  <xsl:template name="group-3-template">
    ...
    <div class="block-group">
      <xsl:call-template name="text-input">
        <xsl:with-param name="name" select="concat('Entries-2[', $position ,'].entries2_required')"/>
        <xsl:with-param name="value" select="$select-node/required2"/>
        <xsl:with-param name="label" select="'Required'"/>
        <xsl:with-param name="required" select="true()"/>
      </xsl:call-template>
      <xsl:call-template name="text-input">
        <xsl:with-param name="name" select="concat('Entries-2[', $position ,'].entries2_unrequired')"/>
        <xsl:with-param name="value" select="$select-node/unrequired"/>
        <xsl:with-param name="label" select="'Unrequired'"/>
        <xsl:with-param name="required" select="false()"/>
      </xsl:call-template>
