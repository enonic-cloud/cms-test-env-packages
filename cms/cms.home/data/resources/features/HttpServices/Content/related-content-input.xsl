...
<xsl:stylesheet exclude-result-prefixes="#all" version="2.0"
  ...
  <xsl:param name="category-key" select="90"/>
      ...
      <form action="{$form-action}" method="post" class="cty-form">
        <div>
          ...
          <fieldset id="fieldset-1">
            ...
            <xsl:call-template name="dropdown-input">
              <xsl:with-param name="name" select="'required'"/>
              <xsl:with-param name="value" select="$selected-content/contentdata/required/@key"/>
  ...
  <xsl:template name="group-2-template">
    ...
    <div class="block-group">
      <xsl:call-template name="dropdown-input">
        ...
        <xsl:with-param name="value" select="$select-node/required/@key"/>
        <xsl:with-param name="helper-content" select="/result/helper-content/contents/content"/>
        <xsl:with-param name="label" select="'Required'"/>
        <xsl:with-param name="required" select="true()"/>
      </xsl:call-template>
      <xsl:call-template name="dropdown-input">
        ...
        <xsl:with-param name="value" select="$select-node/unrequired/@key"/>
        <xsl:with-param name="helper-content" select="/result/helper-content/contents/content"/>
  ...
  <xsl:template name="group-3-template">
    ...
    <div class="block-group">
      <xsl:call-template name="dropdown-input">
        ...
        <xsl:with-param name="value" select="$select-node/required2/@key"/>
        <xsl:with-param name="helper-content" select="/result/helper-content/contents/content"/>
        <xsl:with-param name="label" select="'Required'"/>
        <xsl:with-param name="required" select="true()"/>
      </xsl:call-template>
