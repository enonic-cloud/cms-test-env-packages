...
<xsl:stylesheet exclude-result-prefixes="#all" version="2.0"
  ...
  <xsl:param name="category-key" select="94"/>
  ...
  <xsl:template name="group-2-template">
    ...
    <div class="block-group">
      <xsl:call-template name="radiobutton-input">
        ...
        <xsl:with-param name="options" select="'Ost:ost, Fisk:fisk'"/>
      ...
      <xsl:call-template name="radiobutton-input">
        ...
        <xsl:with-param name="options" select="'Manila:Manila,Marseille:Marseille,Rio de Janeiro:Rio de Janeiro'"/>
        <xsl:with-param name="label" select="'Unrequired'"/>
        <xsl:with-param name="required" select="false()"/>
      </xsl:call-template>
      <br style="clear:both;"/>
  ...
  <xsl:template name="group-3-template">
    ...
    <div class="block-group">
      <xsl:call-template name="radiobutton-input">
        ...
        <xsl:with-param name="options" select="'Torsk:Torsk, Sei:Sei, Gjedde:Gjedde'"/>
        <xsl:with-param name="label" select="'Required'"/>
        <xsl:with-param name="required" select="true()"/>
      </xsl:call-template>
      <xsl:call-template name="radiobutton-input">
        ...
        <xsl:with-param name="value" select="$select-node/unrequired"/>
        <xsl:with-param name="options" select="'2. Februar:2. Februar,21. April:21. April,11. Desember:11. Desember'"/>
        <xsl:with-param name="label" select="'Unrequired'"/>
        <xsl:with-param name="required" select="false()"/>
      </xsl:call-template>
      <br style="clear:both;"/>
