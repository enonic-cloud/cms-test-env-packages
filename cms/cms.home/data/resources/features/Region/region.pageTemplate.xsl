...
<xsl:stylesheet exclude-result-prefixes="xs portal"
          ...
          <div>
            ...
            <div class="content">
              <div class="inner-content">
                ...
                <p>
                  <xsl:choose>
                    <xsl:when test="$loadPortletWindowsLazy">
                      <a href="{portal:createPageUrl( ('lazyLoadPortlets', 'false') )}">Click here to load the portlet
                        windows inline
                      </a>
                    </xsl:when>
                    <xsl:otherwise>
                      <a href="{portal:createPageUrl( ('lazyLoadPortlets', 'true') )}">Click here to load the portlet
                        windows lazy with Ajax
                      </a>
                    </xsl:otherwise>
                  </xsl:choose>
                </p>
                ...
                <div id="rightColumn">
                  <p>
                    Here we display the portlet windows in the region named 'rightColumn':
                    <xsl:call-template name="displayRegion">
                      <xsl:with-param name="regionName" select="'rightColumn'"/>
                      <xsl:with-param name="lazyLoad" select="$loadPortletWindowsLazy"/>
                    </xsl:call-template>
  ...
  <xsl:template name="displayRegion">
    ...
    <xsl:choose>
      <xsl:when test="$lazyLoad">
        <xsl:for-each select="/result/context/page/regions/region[ name = $regionName ]/windows/window">

          <div id="window-{portlet/@key}">
            Please wait.... loading
          </div>

          <xsl:variable name="portletUrl" select="portal:createWindowUrl( @key, ('fisk', 'ost') )"/>
          <script type="text/javascript">

            setTimeout( function() {
            $('#window-<xsl:value-of select="portlet/@key"/>').load('<xsl:value-of select="$portletUrl"/>');}, 500 );
