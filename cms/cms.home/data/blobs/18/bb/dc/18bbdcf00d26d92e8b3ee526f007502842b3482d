...
<xsl:stylesheet exclude-result-prefixes="#all" version="2.0" xmlns="http://www.w3.org/1999/xhtml" xmlns:portal="http://www.enonic.com/cms/xslt/portal" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xs="http://www.w3.org/2001/XMLSchema" xmlns:local="enonic:local">
  ...
  <xsl:template match="/">
    <xsl:if test="/result/page/html != ''">
      <style type="text/css">
        @import url('<xsl:value-of select="portal:createResourceUrl('/_public/modules/confluence/sh/styles/shCore.css')" />');
        @import url('<xsl:value-of select="portal:createResourceUrl('/_public/modules/confluence/sh/styles/shThemeDefault.css')" />');
      </style>
      <script type="text/javascript" src="{portal:createResourceUrl('/_public/modules/confluence/sh/scripts/shCore.js')}"/>
      <script type="text/javascript" src="{portal:createResourceUrl('/_public/modules/confluence/sh/scripts/shAutoloader.js')}"/>
      <script type="text/javascript" src="{portal:createResourceUrl('/_public/modules/confluence/scripts/confluence.js')}"/>
      <!-- Autoloading script for SyntaxHighlighting -->
      <script type="text/javascript">
        $(document).ready(function() {
          var baseUrl = '<xsl:value-of select="portal:createResourceUrl('/_public/modules/confluence/sh/scripts/')" />';
          //Remove timestamp for correct url
          var url =  baseUrl.split('?_ts')[0];
      ...
      <xsl:variable name="test">
        <xsl:analyze-string select="/result/page/html" regex="(script)">
          <xsl:matching-substring>pre</xsl:matching-substring>
