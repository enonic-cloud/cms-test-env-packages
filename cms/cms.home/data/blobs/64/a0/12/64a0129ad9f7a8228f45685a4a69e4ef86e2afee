...
<xsl:stylesheet exclude-result-prefixes="#all" version="2.0"
  ...
  <xsl:param name="category-key" select="106"/>
          ...
          <fieldset id="fieldset-1">
            ...
            <div id="required-files">
              <p class="add-block-group-button-container">
                [ <a href="javascript:;" onclick="addBlockGroup($('div#block-group-1-template'), $('#required-files'))">Add image</a> ]
              </p>
              <xsl:for-each select="$selected-content/contentdata/required/image">
            ...
            <div id="unrequired-files">
              <p class="add-block-group-button-container">
                [ <a href="javascript:;" onclick="addBlockGroup($('div#block-group-2-template'), $('#unrequired-files'))">Add image</a> ]
              </p>
              <xsl:for-each select="$selected-content/contentdata/unrequired/image">
  ...
  <xsl:template name="group-1-template">
    ...
    <div class="block-group no-border">
      <xsl:call-template name="images-input">
        <xsl:with-param name="name" select="'images_required'"/>
  ...
  <xsl:template name="group-2-template">
    ...
    <div class="block-group no-border">
      <xsl:call-template name="images-input">
        <xsl:with-param name="name" select="'images_unrequired'"/>
        <xsl:with-param name="value" select="$select-node"/>
        <xsl:with-param name="helper-content" select="/result/helper-content/contents/content"/>
