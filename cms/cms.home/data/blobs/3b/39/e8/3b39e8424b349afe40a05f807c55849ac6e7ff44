<xsl:stylesheet exclude-result-prefixes="#all" version="2.0" xmlns="http://www.w3.org/1999/xhtml" 
  ...
  <!--<xsl:variable name="toplevel" as="element()*" select="$config-device-class/menu/toplevels/toplevel"/> added for Travels-->
  ...
  <xsl:template name="page.page">
    <html xmlns="http://www.w3.org/1999/xhtml" dir="ltr" lang="{$language}" xml:lang="{$language}">
      <head>
        ...
        <link rel="shortcut icon" type="image/x-icon" href="{portal:createResourceUrl(concat($site-public, '/themes/travel/images/favicon.ico'))}"/>
  ...
  <xsl:template name="page.header">
    <div id="header">
      <a class="screen" href="{portal:createUrl($front-page)}">
        <img alt="{$site-name}-{portal:localize('logo')}" id="logo-screen" src="{portal:createResourceUrl(concat($theme-public, '/themes/travel/images/logo-screen.png'))}" title="{$site-name}"/>
      </a>
      <img alt="{$site-name}-{portal:localize('logo')}" id="logo-print" class="print" src="{portal:createResourceUrl(concat($theme-public, '/themes/travel/images/logo-print.gif'))}" title="{$site-name}"/>
                ...
                <li>
                  <a href="{portal:createPageUrl(@key, ())}" class="tooltip" title="{parameters/parameter[@name = 'display-name']}">
                    <xsl:variable name="flag">
                      <xsl:value-of select="concat(concat($path-to-skin, '/themes/travel/images/flag-'), name)"/>
  ...
  <xsl:template name="page.footer">
    <div id="footer">
      <img alt="{$site-name}-{portal:localize('logo')}" src="{portal:createResourceUrl(concat($path-to-skin, '/themes/travel/images/logo-footer.png'))}" title="{$site-name}" class="screen"/>
      <xsl:if test="$rss-page">
        <a href="{portal:createUrl($rss-page)}" class="screen">
          <img src="{portal:createResourceUrl(concat($path-to-skin, '/themes/travel/images/icon-rss-large.png'))}" alt="RSS {portal:localize('icon')}"/>
        </a>
      </xsl:if>
      <p>
      ...
      <a href="{portal:createServicesUrl('portal','forceDeviceClass', ('deviceclass', 'mobile', 'lifetime', 'session'))}" class="device-class screen" rel="nofollow">
        <img src="{portal:createResourceUrl(concat($path-to-skin, '/themes/travel/images/icon-mobile.png'))}" alt="{portal:localize('Mobile-version')}" class="icon text"/>
