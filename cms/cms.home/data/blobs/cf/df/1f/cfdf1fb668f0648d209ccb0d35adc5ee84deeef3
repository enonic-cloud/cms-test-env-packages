...
<xsl:stylesheet exclude-result-prefixes="#all" version="2.0" xmlns:portal="http://www.enonic.com/cms/xslt/portal" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:variable name="list-id" select="concat('flickr-gallery-list', /verticaldata/context/portlet-window/portlet/@key)"/>

    <xsl:template match="/">
        <xsl:variable name="feed-provider">
            <xsl:choose>
                <xsl:when test="matches(lower-case($feed-url), 'twitter')">twitter</xsl:when>
                <xsl:when test="matches(lower-case($feed-url), 'brightkite')">brightkite</xsl:when>
                <xsl:when test="matches(lower-case($feed-url), 'facebook')">facebook</xsl:when>
                <xsl:otherwise>unknown</xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        <div class="{concat('social-updates ', $feed-provider)}">
            <h2>
                <xsl:value-of select="portal:localize('Follow-us-on', ($feed-provider))"/>
            </h2>
            <ul id="{$list-id}">
                <li class="system">
        ...
        <script type="text/javascript">
            <xsl:comment>
                $(function () {
                    ...
                    $('#' + listId + ' a.refresh-content').click(function (event) {
