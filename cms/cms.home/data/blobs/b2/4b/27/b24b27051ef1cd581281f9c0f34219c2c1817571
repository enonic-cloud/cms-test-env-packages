...
<xsl:stylesheet exclude-result-prefixes="#all" version="2.0"
  ...
  <xsl:param name="category-key" select="91"/>
  ...
  <xsl:template match="/">
    <div>
      ...
      <form action="{$form-action}" method="post" class="cty-form" enctype="multipart/form-data">
        <div>
          ...
          <fieldset id="fieldset-1">
            ...
            <xsl:call-template name="uploadfile-input">
              <xsl:with-param name="name" select="'required'"/>
              <xsl:with-param name="content-key" select="$selected-content/@key"/>
              <xsl:with-param name="binary-key" select="$selected-content/contentdata/required/binarydata/@key"/>
              <xsl:with-param name="label" select="'Required'"/>
              <xsl:with-param name="required" select="true()"/>
            </xsl:call-template>
            <xsl:call-template name="uploadfile-input">
              <xsl:with-param name="name" select="'unrequired1'"/>
              <xsl:with-param name="content-key" select="$selected-content/@key"/>
              <xsl:with-param name="binary-key" select="$selected-content/contentdata/unrequired1/binarydata/@key"/>
              <xsl:with-param name="label" select="'Unrequired 1'"/>
              <xsl:with-param name="required" select="false()"/>
            </xsl:call-template>
            <xsl:call-template name="uploadfile-input">
              <xsl:with-param name="name" select="'unrequired2'"/>
              <xsl:with-param name="content-key" select="$selected-content/@key"/>
              <xsl:with-param name="binary-key" select="$selected-content/contentdata/unrequired2/binarydata/@key"/>
              <xsl:with-param name="label" select="'Unrequired 2'"/>
              <xsl:with-param name="required" select="false()"/>
            </xsl:call-template>
          </fieldset>
  ...
  <xsl:template name="group-2-template">
    ...
    <div class="block-group">
      <xsl:call-template name="uploadfile-input">
        ...
        <xsl:with-param name="content-key" select="$selected-content/@key"/>
        <xsl:with-param name="binary-key" select="$select-node/required/binarydata/@key"/>
        <xsl:with-param name="label" select="'Required'"/>
        <xsl:with-param name="required" select="true()"/>
      </xsl:call-template>
      <xsl:call-template name="uploadfile-input">
        ...
        <xsl:with-param name="content-key" select="$selected-content/@key"/>
        <xsl:with-param name="binary-key" select="$select-node/unrequired/binarydata/@key"/>
  ...
  <xsl:template name="group-3-template">
    ...
    <div class="block-group">
      <xsl:call-template name="uploadfile-input">
        ...
        <xsl:with-param name="content-key" select="$selected-content/@key"/>
        <xsl:with-param name="binary-key" select="$select-node/required2/binarydata/@key"/>
        <xsl:with-param name="label" select="'Required'"/>
        <xsl:with-param name="required" select="true()"/>
      </xsl:call-template>
      <xsl:call-template name="uploadfile-input">
