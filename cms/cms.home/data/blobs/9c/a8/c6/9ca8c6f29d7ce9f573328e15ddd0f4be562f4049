...
<xsl:stylesheet exclude-result-prefixes="#all" version="2.0"
  ...
  <xsl:param name="category-key" select="107"/>
      ...
      <form action="{$form-action}" method="post" class="cty-form">
        <div>
          ...
          <fieldset id="fieldset-1">
            ...
            <xsl:call-template name="file-input">
              <xsl:with-param name="name" select="'unrequired'"/>
              <xsl:with-param name="value" select="$selected-content/contentdata/unrequired/file/@key"/>
  ...
  <xsl:template name="group-2-template">
    ...
    <div class="block-group">
      <xsl:call-template name="file-input">
        ...
        <xsl:with-param name="value" select="$select-node/required/file/@key"/>
        <xsl:with-param name="helper-content" select="/result/helper-content/contents/content"/>
      ...
      <xsl:call-template name="file-input">
        ...
        <xsl:with-param name="value" select="$select-node/unrequired/file/@key"/>
        <xsl:with-param name="helper-content" select="/result/helper-content/contents/content"/>
        <xsl:with-param name="label" select="'Unrequired'"/>
        <xsl:with-param name="required" select="false()"/>
      </xsl:call-template>
  ...
  <xsl:template name="group-3-template">
    ...
    <div class="block-group">
      <xsl:call-template name="file-input">
        ...
        <xsl:with-param name="value" select="$select-node/required2/file/@key"/>
        <xsl:with-param name="helper-content" select="/result/helper-content/contents/content"/>
        <xsl:with-param name="label" select="'Required'"/>
        <xsl:with-param name="required" select="true()"/>
      </xsl:call-template>
