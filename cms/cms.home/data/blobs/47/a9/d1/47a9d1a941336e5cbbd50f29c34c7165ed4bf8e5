...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml"
    ...
    <xsl:template match="/">
        ...
        <ul>
            ...
            <li><xsl:value-of select="/result/plugin-cached1/value"/> (from plugin)</li>
        </ul>

        <h3>Cached 2</h3>
        <ul>
            <xsl:apply-templates select="/result/cached2"/>
            <li><xsl:value-of select="/result/plugin-cached2/value"/> (from plugin)</li>
        </ul>
        ...
        <ul>
            ...
            <li><xsl:value-of select="/result/plugin-not-cached1/value"/> (from plugin)</li>
        </ul>

        <h3>Not-Cached 2</h3>
        <ul>
            <xsl:apply-templates select="/result/not-cached2"/>
            <li><xsl:value-of select="/result/plugin-not-cached2/value"/> (from plugin)</li>
        </ul>
